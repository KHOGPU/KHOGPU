...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Khorium database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Khorium datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Khorium datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Khorium 0.7 datadir."
    ;;
  3)
    echo "Detected Khorium pre-0.8 datadir."
    ;;
  4)
    echo "Detected Khorium 0.8 datadir."
    ;;
esac
